package com.t_assistant.database;
...
public class DatabaseHandler {
    ...
    public DatabaseHandler(Activity activity) {
        ...
        database = activity.openOrCreateDatabase("teacher", activity.MODE_PRIVATE, null);
        createTable();
    }
    ...
    public void createTable() {
        try {
            String qu = "CREATE TABLE IF NOT EXISTS STUDENT(name varchar(1000)," +
                    "cl varchar(100), " +
                    "regno VARCHAR(100) primary key, contact varchar(100),roll integer UNIQUE);";
            database.execSQL(qu);
        } catch (Exception e) {
