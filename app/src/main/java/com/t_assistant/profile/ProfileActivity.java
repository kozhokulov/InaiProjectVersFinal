package com.t_assistant.profile;
...
public class ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                alert.setMessage("Подтвердите действие");
                alert.setPositiveButton("Да", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        EditText editText = (EditText) findViewById(R.id.editText);
                        String regno = editText.getText().toString();
                        String qu = "DELETE FROM STUDENT WHERE age = '" + regno.toUpperCase() + "'";
    ...
    public void find(View view) {
        ...
        String qu = "SELECT * FROM STUDENT WHERE roll = '" + reg.toUpperCase() + "'";
        ...
        if (cursor == null || cursor.getCount() == 0) {
            assert textView != null;
            textView.setText("Нет посещаемости");
        } else {
            String attendance = "";
            if (att < 0) {
                attendance = "Посещаемость недоступна";
            } else
            ...
            if (cursorx == null || cursorx.getCount() == 0) {
                Toast.makeText(getBaseContext(), "Нет доступной информации по посещаемости", Toast.LENGTH_LONG).show();
            } else {
                cursorx.moveToFirst();
