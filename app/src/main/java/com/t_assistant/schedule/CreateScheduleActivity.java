...
public class CreateScheduleActivity extends AppCompatActivity {
    ...
    private void saveSchedule(View v) {
        ...
        if (subject.length() < 2) {
            Toast.makeText(getBaseContext(), "Введите правильный предмет", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (AppBase.handler.execAction(sql)) {
            Toast.makeText(getBaseContext(), "Расписание готово", Toast.LENGTH_LONG).show();
            this.finish();
        } else
